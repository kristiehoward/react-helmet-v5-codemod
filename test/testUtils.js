...
/**
 * NOTE:
 * This was copied over from https://github.com/facebook/jscodeshift/blob/master/src/testUtils.js,
 * but is modified so that it can use jest snapshot testing and certain errors.
 * The below LICENSE file can be found at https://github.com/facebook/jscodeshift.
 */
 ...
 */
 // NOTE: Added error here
function runTest(dirName, transformName, options, testFilePrefix, error) {
  if (!testFilePrefix) {
  ...
  if (error) {
    expect(output).toThrowError(error);
  } else {
 ...
 */
function defineTest(dirName, transformName, options, testFilePrefix, error) {
  const testName = testFilePrefix
    ? error
    ? `errors correctly using "${testFilePrefix} data"`
    : `transforms correctly using "${testFilePrefix}" data`
    : 'transforms correctly';
  describe(transformName, () => {
    it(testName, () => {
      runTest(dirName, transformName, options, testFilePrefix, error);
    });
  });
}
